            ...
            <div className={`bg-gradient-to-tl from-violet-800 to-purple-900 rounded-tr-xl rounded-br-xl justify-center p-1 ${contextUser.role != "User" ? "blur-[10px]" : ""}`}>
              <div className="w-full h-full bg-[#5700a2] overflow-x-clip p-4 flex flex-col items-center rounded-tr-xl rounded-br-xl">
                ...
                <nav className="mt-4 space-y-2 text-center">
                  <Link disabled={contextUser.role == "Guest" ? true : false} route={`/profile/${contextUser.username}/watching`} className="border-b-2 border-indigo-950 pb-2 block text-white text-sm hover:underline">
                    Currently Watching
                  </Link>
                  <Link disabled={contextUser.role == "Guest" ? true : false} route={`/profile/${contextUser.username}/watched`} className="border-b-2 border-indigo-950 pb-2 block text-white text-sm hover:underline">
                    Watched
                  </Link>
